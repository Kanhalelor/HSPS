...
const itemToCartWarningMsg = document.querySelector('.item-already-in-cart');
const closeCartMsg = document.querySelector('.close-msg');
...
const addItemToCart = (productName, productPrice, productImage) => {
    ...
    for (let i = 0; i < cartItemNames.length; i++) {
        if (cartItemNames[i].innerText == productName) {
            itemToCartWarningMsg.style.visibility = 'visible';
            return;
        }
    }
    const itemTemplate = `
...
const closeWarningMsg = (e) => {
    e.target.parentElement.style.visibility = 'hidden';
}
// other events
cart.addEventListener('click', displayCart);
closeCartIcon.addEventListener('click', closeCart);
closeCartMsg.addEventListener('click', closeWarningMsg);